...
class LoginAfterJoinActivity : AppCompatActivity() {
            ...
            call.enqueue(object : Callback<Void> {
                override fun onResponse(call: Call<Void>, response: Response<Void>) {
                    if (response.isSuccessful) {
                        when (response.code()) {
                            200 -> {
                                Toast.makeText(this@LoginAfterJoinActivity, "로그인 성공", Toast.LENGTH_SHORT).show()
                                ...
                                // 토큰과 userId 로그로 출력
                                Log.d("LoginAfterJoinActivity", "Stored Token: $token")
                                Log.d("LoginAfterJoinActivity", "Stored UserId: $userId")

                                //다음 화면으로 넘어가기
                                val intent = Intent(this@LoginAfterJoinActivity, Register0Activity::class.java)
                                startActivity(intent)
                            }
                            401 -> {
                                Toast.makeText(this@LoginAfterJoinActivity, "로그인 실패: 사용자 ID 또는 비밀번호가 올바르지 않습니다.", Toast.LENGTH_SHORT).show()
                            }
                            else -> {
                                Toast.makeText(this@LoginAfterJoinActivity, "서버 오류: ${response.code()}", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        Toast.makeText(this@LoginAfterJoinActivity, "로그인 실패: 사용자 ID 또는 비밀번호가 올바르지 않습니다.", Toast.LENGTH_SHORT).show()
                ...
                override fun onFailure(call: Call<Void>, t: Throwable) {
                    Toast.makeText(this@LoginAfterJoinActivity, "네트워크 오류: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
        }
    }
